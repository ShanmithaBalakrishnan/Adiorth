...
$redirectTo = SITE_URL.'/Aadhithya.html';
...
if ($request_method == "POST") {
    ...
    if(isset($_POST['contact_submit']))
    {
        ...
        if(count($errors) == 0)
        {
            ...
            $mailSubject =  SITE_NAME.': Mail to Aadhithya';

            /** Send mail to Admin */           
            $adminMailMessage = '<h2> Hi, </h2>
                                <p>Mail from '.$userName.' </p>
                                <p>Phone : '.$phone.' </p>
                                <p>Message : '.$message.' </p>
                                <p>Regards, </p>
                                <p>'.SITE_NAME.' team</p>';
...
function formValidation($postData)
{
    ...
    {
        $fname = sanitizeInput($postData['fname']);
        if (preg_match('/^[A-Za-z\s\-\'\p{L}]+$/u', $fname)) {
            $inputs['fname'] = $fname;
        } else {
